...
// TRBO: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_15_ENABLE_BURN_FEATURE          ) r = SPORK_15_ENABLE_BURN_FEATURE_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_15_ENABLE_BURN_FEATURE") return SPORK_15_ENABLE_BURN_FEATURE;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_15_ENABLE_BURN_FEATURE) return "SPORK_15_ENABLE_BURN_FEATURE";
