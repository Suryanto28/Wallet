...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        if (fProofOfStake) {
            ...
            LogPrintf("TRBOMiner : proof-of-stake block found %s \n", pblock->GetHash().ToString().c_str());
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("TRBOMiner(): Signing new block with UTXO key failed \n");
                return nullptr;
            }
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("trboMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock))
        return error("trboMiner : ProcessNewBlock, block not accepted");
...
void TRBOMiner(CWallet* pwallet, bool fProofOfStake)
{
    LogPrintf("trboMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("trbo-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("TRBOMiner(): Signing new block failed \n");
                continue;
            }

            LogPrintf("TRBOMiner : proof-of-stake block was signed %s \n", pblock->GetHash().ToString().c_str());
        ...
        LogPrintf("Running trboMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadTRBOMiner(void* parg)
{
    boost::this_thread::interruption_point();
    CWallet* pwallet = (CWallet*)parg;
    try {
        TRBOMiner(pwallet, false);
        boost::this_thread::interruption_point();
    } catch (std::exception& e) {
        LogPrintf("ThreadTRBOMiner() exception");
    } catch (...) {
        LogPrintf("ThreadTRBOMiner() exception");
    }

    LogPrintf("ThreadTRBOMiner exiting\n");
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadTRBOMiner, pwallet));
}
